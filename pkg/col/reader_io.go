...
// readBytesAt reads bytes from the file at a specific offset
func (r *Reader) readBytesAt(offset int64, size int) ([]byte, error) {
	// Create a new buffer for each read - this is safer for concurrent access
	// than using a buffer pool which could lead to data races
	buf := make([]byte, size)
	...
	// If we didn't read the full buffer, trim it
	if n < size {
		buf = buf[:n]
	}
